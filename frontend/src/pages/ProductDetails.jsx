...
const resolveImageUrl = (value) => {
  ...
  // Convert both relative and full old GridFS URLs.
  ...
  // Normalize current GridFS URLs to the configured backend origin.
  if (url.includes("/api/catalog/images/")) {
    const imageId = url.split("/api/catalog/images/")[1];
  ...
  if (Array.isArray(product.imageFiles)) {
    [...product.imageFiles]
      ...
      .forEach((item) => {
        ...
        if (fileId) {
          addImage(
            `${API_BASE}/api/catalog/images/${String(fileId)}`,
          );
        } else if (item?.url) {
          addImage(item.url);
        }
...
*    ===================================================== */
    ...
    if (response.ok) {
      ...
      loadedProduct = normalizeProduct(
...
*     LOAD ALL PRODUCTS AND FIND THIS ID*



*     This fixes imported ZIP products when*

*     /api/catalog/products/:id is not available.*
...
*  ===================================================== */
  ...
  if (!loadedProduct) {
    ...
    console.log(
      ...
      "Trying product-list fallback for:",
    ...
    console.log(
      ...
      "All products for detail lookup:",
    ...
    try {
      ...
      if (!allResponse.ok) {
        ...
        throw new Error(
          ...
          `Suggested products API failed (${allResponse.status})`
      ...
      const allProducts =
        ...
        Array.isArray(allData)
          ...
          : Array.isArray(allData?.products)
            ...
            : Array.isArray(allData?.data?.products)
      ...
      console.error(
        ...
        "Suggested products could not load:",
