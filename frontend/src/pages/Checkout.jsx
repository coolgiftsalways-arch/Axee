...
function Checkout() {
  ...
  const handleSubmit = async (event) => {
    ...
    const checkoutPayload = {
      ...
      paymentMethod,
      codAdvanceRequired,
      advancePercentage: codAdvanceRequired ? 10 : 0,
      advanceAmount: codAdvanceAmount,
      balanceDueOnDelivery: paymentMethod === "cod" ? codBalanceAmount : 0,
      items,
      subtotal,
      shipping,
      total,
    ...
    /*
      FRONTEND CHECKOUT IS READY.

      For a real order/payment:
      1. Send checkoutPayload to your backend order endpoint.
      2. For UPI/Card, your backend should create a Razorpay order.
      3. Open Razorpay Checkout using the returned order ID.
      4. Verify the payment signature in the backend.
      5. Save the final order in MongoDB.

      Send me your backend order/payment files and I can connect this
      page to your existing backend without guessing your API routes.
    */

    try {
      setPlacingOrder(true);

      console.log("UNBOUND checkout payload:", checkoutPayload);

      if (codAdvanceRequired) {
        // IMPORTANT:
        // Do NOT confirm this order on the frontend.
        // Your backend must create a Razorpay order for codAdvanceAmount,
        // verify the successful payment signature, and only then create /
        // confirm the COD order with codBalanceAmount left to collect.
        window.alert(
          `For Cash on Delivery orders of ₹2,000 or more, a 10% advance of ${money(
            codAdvanceAmount,
          )} must be paid online first. After successful payment, the remaining ${money(
            codBalanceAmount,
      ...
      // Temporary frontend-only completion for the current demo flow.
      // Replace this with your real order API.
    ...
    return (
      <main className="ax-checkout-page ax-checkout-success-page">
        ...
        <section className="ax-checkout-success">
          ...
          <span className="ax-checkout-eyebrow">CHECKOUT FORM COMPLETE</span>
          ...
          <p>
            The checkout UI is working. The next step is connecting your backend
            order API and Razorpay so payment and order creation happen for
            real.
          </p>
